...
- `--mode mock`：读取仓库内 `data/mock_samples.json`，零配置；默认 `LLM_SCORING_ENABLED=off` 时零联网，用于验证报告结构与版式。若显式打开 `LLM_SCORING_ENABLED=on` 且配置 DeepSeek key，mock 也可用于 V0.9.7 正式报告语义质检接线验收。
...
- 当前架构基础：**V0.5.0 — 数据源适配层（data-source adapter）**。详见 `docs/changes/0001-data-source-adapter.md`。
  当前运行选择：**V0.9.7 — DeepSeek 正式报告语义质检**，`auto` live 仍默认 `reddit_last30days`；YouTube 只能通过 `scrapecreators_youtube_search` 显式 opt-in。
  注意：V0.9.x 只把 YouTube Search 元数据接入正式 report pipeline，不接字幕、评论或视频画面；V0.9.7 起 DeepSeek 可以在正式报告中做限量语义质检两栏对照，但不当主裁判、不重写整份报告；正式报告生成版本仍保持 V0.6.6。
  ...
  当前 LLM 状态：`scoring/llm_scorer.py`、`scoring/deepseek_client.py`、`config/llm_scoring.json`、`prompts/llm_scoring_prompt.md` 定义评分契约与 DeepSeek 请求保护；`scripts/probe_llm_scoring.sh` 默认 dry-run，不联网。
  DeepSeek 正式报告语义质检只有在 `LLM_SCORING_ENABLED=on` 且 `DEEPSEEK_API_KEY` 可用时运行，默认最多复核 `config/llm_scoring.json` 的 `max_items_per_run` 条候选证据（当前 5 条）；失败只写 `local_outputs/llm_report_review_error.md`，不得阻断或覆盖正式报告。用户已明确允许项目需要时使用 DeepSeek API 额度；搜索引擎额度每次使用前仍必须先向用户申请预计条数并等待批准。
  DeepSeek tiny probe、评分对照报告和真实小样本对照报告作为旁路诊断脚本保留；真实运行仍必须打开 `LLM_SCORING_ENABLED=on` 并加 `--confirm-live-api`，输出只写入 `local_outputs/llm_scoring_probe.*`、`local_outputs/llm_scoring_comparison.*` 或 `local_outputs/llm_scoring_real_sample_comparison.*`。
...
scoring/
  llm_scorer.py               # LLM scoring contract + mock scorer
  deepseek_client.py           # V0.9.7 DeepSeek 正式报告语义质检请求客户端
storage/
...
config/llm_scoring.json       # DeepSeek 正式报告语义质检配置，默认 disabled/formal_report_review
...
LLM 智能评分的正式报告接入仅限 V0.9.7 的 DeepSeek 语义质检两栏对照：`config/llm_scoring.json` 必须默认 `enabled=false`，`.env.example`
必须默认 `LLM_SCORING_ENABLED=off`；打开后每轮最多复核 `max_items_per_run` 条候选证据，不得让 DeepSeek 重写整份报告或直接替代规则主分层。
...
`local_outputs/llm_scoring_comparison.*` 或 `local_outputs/llm_scoring_real_sample_comparison.*`。
...
- `tests/test_llm_report_integration.py`：V0.9.7 DeepSeek 正式报告语义质检的渲染、候选上限、开关/key/错误保护和主流程接入。
...
- `mock` 模式默认 `LLM_SCORING_ENABLED=off` 时不读真实 key、不联网，可覆盖 `reports/report.md`；若显式打开 DeepSeek 正式报告语义质检，则会读取本地 key 并调用 DeepSeek。
...
- DeepSeek 正式报告语义质检默认关闭；打开 `LLM_SCORING_ENABLED=on` 后只做限量两栏对照，不当主裁判，失败只写 `local_outputs/llm_report_review_error.md`，不得阻断报告生成。
