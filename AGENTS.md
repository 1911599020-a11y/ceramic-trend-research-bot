...
- 当前架构基础：**V0.5.0 — 数据源适配层（data-source adapter）**。详见 `docs/changes/0001-data-source-adapter.md`。
  当前运行选择：**V0.6.8 — DeepSeek 大模型评分 tiny probe**，`auto` live 仍默认 `reddit_last30days`；正式报告仍只使用规则评分。
  注意：V0.6.8 是独立 tiny probe 版本，不提升 `ceramic_report.py` 的 `REPORT_VERSION`；正式报告生成版本仍保持 V0.6.6。
  ...
  DeepSeek tiny probe 真实运行必须用户明确同意并加 `--confirm-live-api`，输出只写入 `local_outputs/llm_scoring_probe.*`，不写正式 reports。
...
LLM 智能评分不接入正式报告：`config/llm_scoring.json` 必须默认 `enabled=false`；
`scripts/probe_llm_scoring.sh` 默认不联网，真实 DeepSeek tiny probe 必须用户明确同意并加
`--confirm-live-api`，输出只能写入 `local_outputs/llm_scoring_probe.*`，不得把 LLM 评分写入
...
- DeepSeek LLM scoring probe 默认 dry-run；真实运行必须显式加 `--confirm-live-api`，输出只写入 `local_outputs/llm_scoring_probe.*`。
