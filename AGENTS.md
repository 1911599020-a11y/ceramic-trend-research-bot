...
- 当前架构基础：**V0.5.0 — 数据源适配层（data-source adapter）**。详见 `docs/changes/0001-data-source-adapter.md`。
  当前运行选择：**V0.7.0 — 真实 Reddit/ScrapeCreators 小样本 + DeepSeek 对照报告**，`auto` live 仍默认 `reddit_last30days`；正式报告仍只使用规则评分。
  注意：V0.7.0 是独立旁路对照版本，不提升 `ceramic_report.py` 的 `REPORT_VERSION`；正式报告生成版本仍保持 V0.6.6。
...
scripts/compare_real_llm_scoring.py     # 真实 ScrapeCreators Reddit 小样本 + DeepSeek 旁路对照报告，默认不联网
scripts/compare_real_llm_scoring.sh     # DeepSeek 真实小样本对照 runner
```
...
# DeepSeek 真实 Reddit/ScrapeCreators 小样本对照报告（默认 dry-run，不联网、不消耗 API）
...
- `tests/test_llm_scoring_real_sample_comparison.py`：真实小样本 DeepSeek 对照报告的默认不联网、key 保护、输出保护和成功摘要。
