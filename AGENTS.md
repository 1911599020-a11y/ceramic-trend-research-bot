...
- 当前架构基础：**V0.5.0 — 数据源适配层（data-source adapter）**。详见 `docs/changes/0001-data-source-adapter.md`。
  当前运行选择：**V0.7.3 — 真实样本质量雷达 + DeepSeek 局部质检 + 报告解析**，`auto` live 仍默认 `reddit_last30days`；正式报告仍只使用规则评分。
  注意：V0.7.3 是独立旁路分析版本，不提升 `ceramic_report.py` 的 `REPORT_VERSION`；正式报告生成版本仍保持 V0.6.6。
