...
- 当前架构基础：**V0.5.0 — 数据源适配层（data-source adapter）**。详见 `docs/changes/0001-data-source-adapter.md`。
  当前运行选择：**V0.6.9 — 规则评分 + DeepSeek 评分对照报告**，`auto` live 仍默认 `reddit_last30days`；正式报告仍只使用规则评分。
  注意：V0.6.9 是独立旁路对照版本，不提升 `ceramic_report.py` 的 `REPORT_VERSION`；正式报告生成版本仍保持 V0.6.6。
  ...
  DeepSeek tiny probe 和评分对照报告真实运行必须用户明确同意、打开 `LLM_SCORING_ENABLED=on` 并加 `--confirm-live-api`，输出只写入 `local_outputs/llm_scoring_probe.*` 或 `local_outputs/llm_scoring_comparison.*`，不写正式 reports。
...
`local_outputs/llm_scoring_comparison.*`，不得把 LLM 评分写入
...
- DeepSeek LLM scoring probe 和 scoring comparison 默认 dry-run；真实运行必须显式打开 `LLM_SCORING_ENABLED=on` 并加 `--confirm-live-api`，输出只写入 `local_outputs/llm_scoring_probe.*` 或 `local_outputs/llm_scoring_comparison.*`。
