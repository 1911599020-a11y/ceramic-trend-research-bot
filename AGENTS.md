...
- 当前架构基础：**V0.5.0 — 数据源适配层（data-source adapter）**。详见 `docs/changes/0001-data-source-adapter.md`。
  当前运行选择：**V0.6.6 — 小批量关键词质量测试工具**，`auto` live 仍默认 `reddit_last30days`；ScrapeCreators 正式 live 优先用 `scripts/run_scrapecreators_live.sh`，关键词质量测试优先用 `scripts/run_keyword_quality_check.sh`，默认 dry-run，不联网。
  下一步计划：在用户明确同意消耗 API 额度后，运行小批量关键词质量测试，评估 `kiln firing`、`ceramic business`、`AI ceramic design`。
  V0.6.7 预留方向：报告可读性优化要做成“报告 + 解析”，即正式报告面向陶瓷工作室快速阅读，解析部分负责解释为什么这么判断、证据强弱、关键词质量和下一步行动。
