...
- 当前架构基础：**V0.5.0 — 数据源适配层（data-source adapter）**。详见 `docs/changes/0001-data-source-adapter.md`。
  当前运行选择：**V0.8.0 — YouTube tiny probe**，`auto` live 仍默认 `reddit_last30days`；正式报告仍只使用规则评分。
  注意：V0.8.0 是独立旁路探针版本，不提升 `ceramic_report.py` 的 `REPORT_VERSION`；正式报告生成版本仍保持 V0.6.6。
...
`youtube_future` 继续保持 planned，V0.8.0 不把 YouTube 接入正式报告，也不接 DeepSeek。
...
陶瓷机会雷达、共同数据库和多语言输出属于后续产品化方向，不应在未规划前打乱 V0.8.0 YouTube
tiny probe 主线。
