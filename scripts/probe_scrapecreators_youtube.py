#!/usr/bin/env python3
"""Tiny opt-in ScrapeCreators YouTube Search probe.

This script is deliberately separate from the formal report pipeline. It only
checks whether one tiny ScrapeCreators YouTube Search request can work, and it
writes sanitized output under local_outputs/.
"""
...
DEFAULT_QUERY = "ceramic glaze"
DEFAULT_SUMMARY_LIMIT = 3
MAX_SUMMARY_LIMIT = 3
DEFAULT_TIMEOUT_SECONDS = 12
USER_AGENT = "ceramic-trend-research-bot/0.8.0"
EXPECTED_OUTPUTS = {
    "state-file": LOCAL_OUTPUTS_DIR / "youtube_probe_state.json",
    "output": LOCAL_OUTPUTS_DIR / "youtube_probe.json",
    "error-file": LOCAL_OUTPUTS_DIR / "youtube_probe_error.md",
}
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run a tiny opt-in ScrapeCreators YouTube Search probe. Without "
    ...
    parser.add_argument("--sort-by", default="relevance", choices=["relevance", "popular"])
    parser.add_argument(
        "--type",
        default="videos",
    ...
    parser.add_argument("--state-file", default="local_outputs/youtube_probe_state.json")
    parser.add_argument("--output", default="local_outputs/youtube_probe.json")
    parser.add_argument("--error-file", default="local_outputs/youtube_probe_error.md")
...
def write_error_markdown(
    ...
    *,
    ...
    text = "\n".join(
        [
            "# ScrapeCreators YouTube Tiny Probe Error",
            "",
            f"- 错误类型：{state.get('error_type', 'unknown_error')}",
            f"- 发生时间：{state.get('requested_at', '')}",
            f"- 搜索关键词：{state.get('query', '')}",
            f"- 保存上限：{state.get('summary_limit', '')}",
...
def build_request_url(query: str, upload_date: str, sort_by: str, item_type: str) -> str:
...
def request_scrapecreators(
    *,
    ...
    req = request.Request(
        build_request_url(query, upload_date, sort_by, item_type),
        headers={
            "x-api-key": api_key,
...
def channel_name(raw: Mapping[str, Any]) -> str:
    ...
    return safe_text(first_present(raw, ("channelName", "channelTitle", "channel", "author")), max_length=120)
...
def video_url(raw: Mapping[str, Any]) -> str:
    for key in ("url", "videoUrl", "link"):
        ...
        if value:
            return value
    video_id = safe_text(first_present(raw, ("videoId", "id")), max_length=120)
    if video_id:
        return f"https://www.youtube.com/watch?v={video_id}"
    return ""


def summarize_video(raw: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "title": safe_text(first_present(raw, ("title", "headline")), max_length=200),
        "channel": channel_name(raw),
        "url": video_url(raw),
        "video_id": safe_text(first_present(raw, ("videoId", "id")), max_length=120),
        "published": safe_text(first_present(raw, ("publishedTimeText", "published", "publishedAt", "date")), max_length=120),
...
def collect_items(payload: Mapping[str, Any], item_type: str) -> tuple[list[Mapping[str, Any]], dict[str, int]]:
    counts: dict[str, int] = {}
    collected: list[Mapping[str, Any]] = []
    for key in ("videos", "shorts", "channels", "playlists", "lives"):
        value = payload.get(key, [])
        count = len(value) if isinstance(value, list) else 0
        counts[key] = count
        if key == item_type and isinstance(value, list):
            collected.extend(item for item in value if isinstance(item, Mapping))
    if not collected and item_type == "videos":
        fallback = payload.get("items", [])
        if isinstance(fallback, list):
            collected.extend(item for item in fallback if isinstance(item, Mapping))
            counts["items"] = len(fallback)
    return collected, counts


def sanitize_videos(payload: Mapping[str, Any], item_type: str, summary_limit: int) -> tuple[list[dict[str, Any]], dict[str, int]]:
    items, counts = collect_items(payload, item_type)
    return [summarize_video(raw) for raw in items[:summary_limit]], counts


def response_shape_notes(payload: Mapping[str, Any]) -> list[str]:
    notes: list[str] = []
    for key in ("videos", "shorts", "channels", "playlists", "lives", "continuationToken"):
        notes.append(f"{key}: {'present' if key in payload else 'missing'}")
    for key in ("videos", "shorts", "channels", "playlists", "lives"):
        value = payload.get(key)
        if key in payload and not isinstance(value, list):
            notes.append(f"{key} is not a list")
    return notes
...
def success_summary(
    *,
    ...
    videos, counts = sanitize_videos(payload, item_type, summary_limit)
    return {
        "source_id": SOURCE_ID,
        "status": "success",
        "endpoint": ENDPOINT,
        ...
        "counts_in_response": counts,
        "summarized_count": len(videos),
        "videos": videos,
...
def failure_summary(
    *,
    ...
    return {
        ...
        "raw_response_saved": False,
        "report_files_updated": False,
        "note": "本次 YouTube probe 失败；请以 state/error 文件为准，正式报告未更新。",
    }
...
def main(
    ...
    *,
    ...
    summary_limit = clamp_summary_limit(args.summary_limit)
    ...
    if not allow_outside_local_outputs:
        ...
        if path_error:
            print("ScrapeCreators YouTube tiny probe：输出路径不安全，未发起网络请求。")
            print(path_error)
    ...
    if not args.confirm_live_api:
        ...
        print("ScrapeCreators YouTube tiny probe：未发起网络请求。")
    ...
    if not readiness.can_attempt_api or not api_key:
        ...
        write_error_markdown(
            ...
            next_step="确认 .env 或系统环境变量里已配置 SCRAPECREATORS_API_KEY，然后再运行 YouTube tiny probe。",
        )
        write_json(
            paths.output_file,
            failure_summary(
        ...
        print("ScrapeCreators YouTube tiny probe：未找到 API key，未发起网络请求。")
    ...
    try:
        ...
        state["counts_in_response"] = summary["counts_in_response"]
        state["summarized_count"] = summary["summarized_count"]
        write_json(paths.output_file, summary)
        write_json(paths.state_file, state)
        print("ScrapeCreators YouTube tiny probe 成功。")
    ...
    next_steps = {
        "unauthorized_401": "检查 API key 是否有效，或 ScrapeCreators 账号是否允许访问 YouTube Search。",
    ...
    print("ScrapeCreators YouTube tiny probe 失败。")
    print("正式报告未更新。")
