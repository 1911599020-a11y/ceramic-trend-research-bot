...
TOPICS_FILE="$ROOT_DIR/config/scrapecreators_probe_topics.json"
DRY_RUN=0
EXTRA_ARGS=()
...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/run_scrapecreators_live.sh [options]

默认只跑 config/scrapecreators_probe_topics.json 的单关键词配置，避免误消耗 API 额度。
...
if [[ "$TOPICS_FILE" == "$ROOT_DIR/config/ceramic_topics.json" ]]; then
  echo "ScrapeCreators live：已显式确认全量关键词运行，可能消耗更多 API 额度。"
else
  echo "ScrapeCreators live：默认安全模式，只运行单关键词配置 config/scrapecreators_probe_topics.json。"
fi
