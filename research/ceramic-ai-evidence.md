...
这条线先在这里做中段收尾：当前项目不继续扩展成自动论文研究系统，主线仍然回到社媒趋势情报工具。

主线结束后，可以把本资料库延伸成一个单独的小工具：

**陶瓷 AI 研究助手 / Ceramic AI Research Product**

后续可探索的能力：

- 自动搜索陶瓷 AI 论文、数据集、项目主页和 GitHub 仓库
- 自动读取 PDF，并抽取研究目标、方法、数据、限制和许可证信息
- 自动判断证据类型：论文证据、用户痛点证据、市场信号、产品假设
- 自动把可信资料写入 `data/research_evidence.json`
- 自动把研究证据同步到报告里的长期方向和下一轮搜索建议

暂缓原因：

- 它会变成一个新的“陶瓷 AI 研究产品”，复杂度高于当前社媒趋势报告主线。
- 当前更重要的是先把 Reddit / YouTube / Pinterest 等真实社媒数据源跑稳。
- 研究证据可以补充专业判断，但不能代替用户讨论和内容趋势。

已保留的继续入口：

- 人读资料入口：`research/ceramic-ai-evidence.md`
- 程序读取入口：`data/research_evidence.json`
- 报告读取参数：`ceramic_report.py --research-evidence`
- 报告关闭参数：`ceramic_report.py --no-research-evidence`
