...
- V0.9.7 起，DeepSeek 可以在正式报告中作为限量语义复核出现
- 本节只做“规则判断 vs DeepSeek 判断”的两栏对照，不替代规则评分主流程
- 必须说明 DeepSeek 语义质检是否开启、是否缺 key、是否失败
- DeepSeek 失败时不能阻断报告生成，也不能覆盖原有规则报告结论
- 每轮复核数量必须受 `config/llm_scoring.json` 的 `max_items_per_run` 限制
...
- 当前本节仍是按关键词生成的初步假设，尚未基于评论、字幕或长文本证据抽取
- 创作者、买家、学生、工作室经营者可能正在抱怨或反复提问的问题
- 尽量区分工艺痛点、内容痛点、商业痛点和工具痛点
- 没有真实长文本证据时，不要把痛点假设写成已验证用户需求
- 后续如果引入评论、字幕或 DeepSeek 生成痛点，必须标明证据来源
