...
V0.7.3 是 **真实 Reddit/ScrapeCreators 小样本质量优化 + DeepSeek 局部质检 + 报告解析版本**，建立在 V0.6.7 智能评分接口设计、V0.6.8 tiny probe、V0.6.8.1 开关保护、V0.6.9 规则对照报告和 V0.7.0 真实小样本对照之上：

- 注意：V0.7.3 仍是独立旁路分析版本，不提升正式报告生成版本；`ceramic_report.py` 的 `REPORT_VERSION` 仍保持 `V0.6.6`，因为正式报告流程没有接入 LLM scoring。
...
当前正式报告只支持 `rules`。V0.7.3 已新增 DeepSeek tiny probe、内置样本评分对照报告、真实 Reddit/ScrapeCreators 小样本对照报告、局部质检和报告解析，但它们都是独立诊断脚本，不接入正式报告流程；日常报告当前不会调用 DeepSeek。
