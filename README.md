...
V0.8.6 是 **YouTube video details tiny probe + DeepSeek 旁路审核准备版本**，建立在 V0.8.3 YouTube Search 审核之上：
...
- V0.8.7-lite 记录了 YouTube 转正判断：当前状态是 `wait`；YouTube 有价值，但仍保持 planned/opt-in，直到 V0.9 最小 adapter 被明确实现。
- `youtube_future` 仍保持 planned，不会被 `--data-source auto` 或正式报告调用；DeepSeek 只参与旁路审核，不写入正式报告。
